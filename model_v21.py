...
class DeepSeekAPI:  # 保持类名不变，以免改动后面的代码
    def __init__(self, api_key=None, provider="qwen"):
        ...
        if self.provider == "qwen":
            self.api_key = api_key or "sk-ws-H.EMRDLHP.scbt.MEQCID5BcfM9aM3gnCQwdh5Auq1zvPPn5Q4ZGyJz7up_WmbiAiADL8iaTj-J7cdsrhAelQAgmckMnZb9nQSuPjopP6wSgA"
            self.api_url = "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions"
            self.model_name = "qwen-max"
            
        # 2. DeepSeek 官方 (国内直连)
        elif self.provider == "deepseek":
            self.api_key = api_key or "sk-8fadd56e91ac4f9d9d2092286ec77cd4"
            self.api_url = "https://api.deepseek.com/chat/completions"
            self.model_name = "deepseek-chat"
            
        # 3. GPT-4o 中转代理 (国内直连，需替换中转平台的URL)
        elif self.provider == "gpt-5.2":
            self.api_key = api_key or "sk-jb-60eafc5fea3cc5fb6543e6611107f38f4484c291aad13e32"
...
def run_optimization_system():
    ...
    api_key_input = input("请输入 API 密钥 (留空使用内置默认 Qwen): ").strip()
    ...
    if api_key_input:
        ...
        elif api_key_input.startswith("sk-8f"): # 匹配你代码里 deepseek 的开头
    ...
    # 💥 正确创建带 provider 的 api 对象
    if not api_key_input:
        api = DeepSeekAPI(provider="qwen") # 留空使用内置默认
    else:
